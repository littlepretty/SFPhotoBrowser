...
-(void)load{
    ...
    if ([self.photoURL hasPrefix:@"http"] || [self.photoURL hasPrefix:@"https"]) {
        ...
        UIImage *image = [UIImage imageWithContentsOfFile:self.photoURL];
        self.iv.image = image;
        self.imageLoaded = YES;
    }
...
-(void)dealloc{
    NSLog(@"object of %@ killed",[self class]);
}


+(UIImage *)snapshot{
    UIWindow *w = [UIApplication sharedApplication].keyWindow.subviews[0];
    CGRect rect = CGRectMake(0, 0,w.bounds.size.width,w.bounds.size.height);
